 ...
 *
 * 1. Реализовать подобие модуля, который позволит генерировать форму пользователем(не разработчиком)
 * в браузере. Далее эта форма отправляется в БД и достается в нужном месте. 2. Валидация.
 * Должна быть универсальной для основных типов полей(има, почта, дата и тд.), а для остальных
 * придумать основные условия и как их применить. 3. Результат работы формы. 1) отправка на почту,
 * 2) запись в БД, 3) вывод на экран. Пользователь должен выбрать 1 из 3-х ваиантов.
 *
 */
class BaseForm {
